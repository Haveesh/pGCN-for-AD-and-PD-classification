...
num_subjects = 970  # Number of subjects
root_folder = '/path/to/data/'
data_folder = os.path.join(root_folder, 'path/to/output')
...
def run_training(adj, features, labels, idx_train, idx_val, idx_test,
    ...
    def train_pGCN1(model_input):
        x = run_training(96, kernel_size=(3, 3), activation='relu', padding = 'same')(model_input)
        x = run_training(96, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(96, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(pool_size=(3, 3), strides = 2)(x)
        x = run_training(192, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(192, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(192, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(pool_size=(3, 3), strides = 2)(x)
        x = run_training(192, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(192, (1, 1), activation='relu')(x)
        x = run_training(10, (1, 1))(x)
    ...
    def train_pGCN2(model_input):
        x = run_training(32, kernel_size=(3, 3), activation='relu', padding = 'same')(model_input)
        x = run_training(64, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(64, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(pool_size=(3, 3), strides = 2)(x)
        x = run_training(32, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(32, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(32, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(pool_size=(3, 3), strides = 2)(x)
        x = run_training(32, (3, 3), activation='relu', padding = 'same')(x)
        x = run_training(64, (1, 1), activation='relu')(x)
        x = run_training(10, (1, 1))(x)
        x = GlobalAveragePooling2D()(x)
        x = Activation(activation='softmax')(x)
    ...
    train_pGCN1_model = train_pGCN1(model_input)
    _, train_pGCN1_weight_file = compile_and_train(train_pGCN1, num_epochs)

    train_pGCN2_model = train_pGCN2(model_input)
    _, train_pGCN1_weight_file = compile_and_train(train_pGCN2, num_epochs)

    train_pGCN3_model = train_pGCN3(model_input)
    _, train_pGCN1_weight_file = compile_and_train(train_pGCN3, num_epochs)
